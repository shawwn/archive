...
INT WINAPI WinMain( HINSTANCE, HINSTANCE, LPSTR, int )
{
	...
	DXUTCreateDevice( D3DADAPTER_DEFAULT, gConfig->GetWindowed(), gConfig->GetWidth(), gConfig->GetHeight(), IsDeviceAcceptable, ModifyDeviceSettings );
...
void InitApp()
{
    ...
    g_HUD.AddButton( IDC_CHANGEDEVICE, L"Change device (F2)", 35, iY, 125, 22 );
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
{
    ...
    //WCHAR str[MAX_PATH];
    //V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"Game.fx" ) );
    ...
    //D3DXVECTOR3 vecEye(0.0f, 300.0f, 0.0f);
    //D3DXVECTOR3 vecAt (1.0f, 0.0f, 0.0f);
    //g_Camera.SetViewParams( &vecEye, &vecAt );

	if((hr = gEngine->Init(pd3dDevice)) != ENG_OK)
		return hr;
                                ...
                                const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
{
	...
	if((hr = gEngine->OnResetDevice(pd3dDevice,pBackBufferSurfaceDesc)) != ENG_OK)
		return hr;
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
{
	...
	V( pd3dDevice->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_ARGB(0, 40, 40, 40), 1.0f, 0) );
...
LRESULT CALLBACK MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, bool* pbNoFurtherProcessing )
{
    ...
    *pbNoFurtherProcessing = g_SampleUI.MsgProc( hWnd, uMsg, wParam, lParam );
	...
	gEngine->MsgProc(hWnd,uMsg,wParam,lParam);
